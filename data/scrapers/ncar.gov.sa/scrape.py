...
# @loggo
@memory.cache(
    ignore=["headers", "cookies"],
    # cache_validation_callback=expires_after(days=30)
)
def request_get_cached_json(url, headers=None, cookies=None):
...
def json_remove_keys(json_obj, keys):
    if isinstance(json_obj, dict):
        return {
            k: json_remove_keys(v, keys) for k, v in json_obj.items() if k not in keys
        }
...
def populate_page_data(page_data):
    ...
    page_data["pdfUrl"] = (
        "https://ncar.gov.sa/api/index.php/resource/"
        + page_data["id"]
    ...
    response_json = request_get_cached_json(
        "https://ncar.gov.sa/api/index.php/api/documents/document/" + page_data["id"],
        headers=headers,
        cookies=cookies,
    )
    page_data["documentData"] = response_json
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use_cached",
        action="store_true",
        help="Will use locally existing pages_data.json and pages_data_populated.json if they already exist",
    )
    parser.add_argument(
        "--output_dir",
        type=Path,
        default="./output",
        help="Output directory to save the scraped data, default is the current directory",
    )
    args = parser.parse_args()

    print("fetching all_release_orgnizations ...")
    response = requests.post(
        "https://ncar.gov.sa/api/index.php/api/documents/all_release_orgnizations/1/1",
        headers=headers,
        cookies=cookies,
    )
    ...
    response = requests.post(
        "https://ncar.gov.sa/api/index.php/api/documents/all_release_orgnizations/1/"
        + str(partial_data["dataLength"]),
        headers=headers,
        cookies=cookies,
    )
    all_release_orgnizations = response.json()
    with open(
        args.output_dir / "all_release_orgnizations.json", "w", encoding="utf8"
    ) as f:
    ...
    if not os.path.exists(args.output_dir / "ar_i18n.json") and not args.use_cached:
        response = requests.get(
            "https://ncar.gov.sa/assets/i18n/ar.json", headers=headers
        )
    ...
    if os.path.exists(args.output_dir / "pages_data.json") and args.use_cached:
        ...
        print("fetching data ...")
        response_json = request_get_cached_json(
            "https://ncar.gov.sa/api/index.php/api/documents/list/1/1/approveDate/DESC",
            headers=headers,
            cookies=cookies,
        )
        partial_data = response_json

        response_json = request_get_cached_json(
            "https://ncar.gov.sa/api/index.php/api/documents/list/1/"
            + str(partial_data["dataLength"])
            + "/approveDate/DESC",
            headers=headers,
            cookies=cookies,
        )
        pages_data = response_json
    ...
    if (
        os.path.exists(args.output_dir / "pages_data_populated.json")
        and args.use_cached
    ):
        with open(
            args.output_dir / "pages_data_populated.json", "r", encoding="utf8"
        ) as f:
            pages_data = json.load(f)
        ...
        with ThreadPool(5) as pool:
            _ = list(
                tqdm(
                    pool.imap(populate_page_data, pages_data["data"]),
        ...
        for page_data in pages_data["data"]:
            ...
            for documentRelation in page_data["documentData"]["data"][
                "documentRelation"
            ]:
                try:
        ...
        if failures:
            print(
                "WARNING: there were failures in processing documentRelation",
                len(failures),
            )
    ...
    percentage_of_conflicts = (
        (len(pages_data["data"]) - len(id2pdfUrl)) / len(pages_data["data"]) * 100
    )
